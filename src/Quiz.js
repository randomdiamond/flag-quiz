...
export default function Quiz() {
  ...
  function checkAnswer(selectedAnswer) {
    answerResultsData
      ? setAnswerResultsData((prevData) => [
          ...prevData,
          {
            answers: answersData.answers,
            correct: answersData.correct_answer.german,
            selected: selectedAnswer.german,
            code: answersData.correct_answer.code,
          },
        ])
      : setAnswerResultsData([
          {
    ...
    selectedAnswer === answersData.correct_answer
      ? setGuessedCorrect({
          correct: true,
          correct_answer: answersData.correct_answer.german,
        })
      : setGuessedCorrect({
          correct: false,
          correct_answer: answersData.correct_answer.german,
      ...
      <div className="quiz-container">
        ...
        {answersData && flagCounter <= 10 && (
          <img
            className="flag"
            alt="flag"
            src={`https://countryflagsapi.com/svg/${answersData.correct_answer.code}`}
          />
        )}
         {answersData && (
          <Points
            deliverResults={deliverResults}
            answerResultsData={answerResultsData}
            guessCorrect={guessedCorrect ? guessedCorrect.correct : undefined}
            flagCounter={flagCounter}
          />
        )}
        {answersData && flagCounter <= 10 && (
          <Answers checkAnswer={checkAnswer} answers={answersData.answers} />
        )}
